...
namespace Expressions.Tests.GenericExpressions
{
    public class GenericExpression_Tests
    {
        ...
        [Fact]
        public void ConditionalExpression_OfGenericType_EvaluatesToFailExpression_WithValue_OfGivenType_OnFalseCondition_Test()
        {
            //Arrange
            var conditionalExpression = new ConditionalExpression<IType, IContext>(new ConstantBool<IContext>(false),
                new ConstantExpression<IType, IContext>(_typeObject1), new ConstantExpression<IType, IContext>(_typeObject2));
            //Act
